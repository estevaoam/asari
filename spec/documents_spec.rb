  ...
  describe "updating the index" do
    ...
    context "when region is not specified" do
      it "allows you to add an item to the index using default region." do
        HTTParty.should_receive(:post).with("http://doc-testdomain.us-east-1.cloudsearch.amazonaws.com/2013-01-01/documents/batch", { :body => [{ "type" => "add", "id" => "1", "fields" => { :name => "fritters"}}].to_json, :headers => { "Content-Type" => "application/json"}})
    ...
    context "when region is specified" do
      ...
      it "allows you to add an item to the index using specified region." do
        HTTParty.should_receive(:post).with("http://doc-testdomain.my-region.cloudsearch.amazonaws.com/2013-01-01/documents/batch", { :body => [{ "type" => "add", "id" => "1", "fields" => { :name => "fritters"}}].to_json, :headers => { "Content-Type" => "application/json"}})
    ...
    it "converts Time, DateTime, and Date fields to timestamp integers for rankability" do
      ...
      HTTParty.should_receive(:post).with("http://doc-testdomain.us-east-1.cloudsearch.amazonaws.com/2013-01-01/documents/batch", { :body => [{ "type" => "add", "id" => "1", "fields" => { :time => Time.at(1333263600).strftime("%Y-%m-%dT%H:%M:%SZ"), :datetime => DateTime.new(2012, 4, 1).strftime("%Y-%m-%dT%H:%M:%SZ"), :date => date.to_time.strftime("%Y-%m-%dT%H:%M:%SZ") }}].to_json, :headers => { "Content-Type" => "application/json"}})
    ...
    it "allows you to delete an item from the index." do
      HTTParty.should_receive(:post).with("http://doc-testdomain.us-east-1.cloudsearch.amazonaws.com/2013-01-01/documents/batch", { :body => [{ "type" => "delete", "id" => "1" }].to_json, :headers => { "Content-Type" => "application/json"}})
